...
const AppPrincipal = {
    state: {
        ...
        listeners: {}, currentView: 'planilha', adminUIDs: {}, userCache: {},
        modal: { isOpen: false, currentWorkoutId: null, currentOwnerId: null, newPhotoUrl: null },
        stravaData: null, currentAnalysisData: null, stravaTokenData: null 
    },
    elements: {},

    init: () => {
        if (typeof window.firebaseConfig === 'undefined') return document.body.innerHTML = "<h1>Erro config.js</h1>";
    ...
    initPlatform: () => {
        AppPrincipal.elements = {
            loader: document.getElementById('loader'), appContainer: document.getElementById('app-container'),
            userDisplay: document.getElementById('userDisplay'), logoutButton: document.getElementById('logoutButton'),
            mainContent: document.getElementById('app-main-content'),
            navPlanilhaBtn: document.getElementById('nav-planilha-btn'), navFeedBtn: document.getElementById('nav-feed-btn'), navProfileBtn: document.getElementById('nav-profile-btn'),
            feedbackModal: document.getElementById('feedback-modal'), closeFeedbackModal: document.getElementById('close-feedback-modal'),
            feedbackModalTitle: document.getElementById('feedback-modal-title'), feedbackForm: document.getElementById('feedback-form'),
            workoutStatusSelect: document.getElementById('workout-status'), workoutFeedbackText: document.getElementById('workout-feedback-text'),
            photoUploadInput: document.getElementById('photo-upload-input'), photoUploadFeedback: document.getElementById('photo-upload-feedback'),
            stravaDataDisplay: document.getElementById('strava-data-display'), saveFeedbackBtn: document.getElementById('save-feedback-btn'),
            commentForm: document.getElementById('comment-form'), commentInput: document.getElementById('comment-input'), commentsList: document.getElementById('comments-list'),
            logActivityModal: document.getElementById('log-activity-modal'), closeLogActivityModal: document.getElementById('close-log-activity-modal'), logActivityForm: document.getElementById('log-activity-form'),
            whoLikedModal: document.getElementById('who-liked-modal'), closeWhoLikedModal: document.getElementById('close-who-liked-modal'), whoLikedList: document.getElementById('who-liked-list'),
            iaAnalysisModal: document.getElementById('ia-analysis-modal'), closeIaAnalysisModal: document.getElementById('close-ia-analysis-modal'), iaAnalysisOutput: document.getElementById('ia-analysis-output'), saveIaAnalysisBtn: document.getElementById('save-ia-analysis-btn'),
            profileModal: document.getElementById('profile-modal'), closeProfileModal: document.getElementById('close-profile-modal'), profileForm: document.getElementById('profile-form'),
            profilePicPreview: document.getElementById('profile-pic-preview'), profilePicUpload: document.getElementById('profile-pic-upload'), profileUploadFeedback: document.getElementById('profile-upload-feedback'),
            profileName: document.getElementById('profile-name'), profileBio: document.getElementById('profile-bio'), saveProfileBtn: document.getElementById('save-profile-btn'),
        ...
        AppPrincipal.state.db.ref('admins/' + user.uid).once('value', adminSnap => {
            ...
            AppPrincipal.state.db.ref('users/' + user.uid).once('value', userSnap => {
                if (userSnap.exists()) {
                    ...
                    AppPrincipal.elements.userDisplay.textContent = AppPrincipal.state.userData.name + (role === 'admin' ? ' (Coach)' : '');
    ...
    navigateTo: (page) => {
        ...
        mainContent.innerHTML = ""; AppPrincipal.cleanupListeners(true); AppPrincipal.state.currentView = page;
    ...
    openProfileModal: () => {
        ...
        if (stravaTokenData && stravaTokenData.accessToken) {
            ...
            stravaSection.innerHTML = `
                <fieldset>
                    <legend><i class='bx bxl-strava'></i> Integração</legend>
                    <p style="margin-bottom:10px; font-size:0.9rem;">Conecte para importar seus treinos.</p>
                    <button id="btn-connect-strava" class="btn btn-secondary" style="background:#fc4c02;color:white;width:100%"><i class='bx bxl-strava'></i> Conectar Strava</button>
                </fieldset>`;
        }
        ...
        if(btnSync) btnSync.onclick = AppPrincipal.handleStravaSyncActivities;
    ...
    openFeedbackModal: (wid, oid, title) => {
        ...
        AppPrincipal.state.db.ref(`data/${oid}/workouts/${wid}`).once('value', s => {
            if(s.exists()) {
                ...
                workoutFeedbackText.value = s.val().feedback || '';
    ...
    handleFeedbackSubmit: async (e) => { 
        ...
        updates[`/data/${currentOwnerId}/workouts/${currentWorkoutId}/status`] = data.status;
        updates[`/data/${currentOwnerId}/workouts/${currentWorkoutId}/feedback`] = data.feedback;
        updates[`/publicWorkouts/${currentWorkoutId}/status`] = data.status;
        updates[`/publicWorkouts/${currentWorkoutId}/feedback`] = data.feedback;
    ...
    handleCommentSubmit: (e) => {
        e.preventDefault();
        if(!AppPrincipal.elements.commentInput.value) return;
    ...
    displayStravaData: (d) => {
        document.getElementById('strava-data-display').innerHTML = `<p>Dist: ${d.distancia}</p><p>Tempo: ${d.tempo}</p><p>Pace: ${d.ritmo}</p>`;
    ...
    openIaAnalysisModal: (data) => {
        ...
        if (data) { iaAnalysisOutput.textContent = data.analysisResult; saveIaAnalysisBtn.classList.add('hidden'); }
        else { iaAnalysisOutput.textContent = "Coletando dados..."; saveIaAnalysisBtn.classList.add('hidden'); AppPrincipal.state.currentAnalysisData = null; }
    },
