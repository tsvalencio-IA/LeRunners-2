/* =================================================================== */
/* PANELS.JS V7.0 - SISRUN CLONE (IA FIX + VISUAL TABLES)
/* =================================================================== */
...
const AdminPanel = {
    ...
    showSection: (section) => {
        ...
        if (section === 'feedbacks') {
            ...
            // CORREÇÃO: Menu de IA que tinha sumido
            area.innerHTML = `
                <div class="section-header">
                    <h3><i class='bx bxs-brain'></i> Inteligência Artificial (KPIs)</h3>
                    <p>Selecione um aluno para que o Gemini analise a evolução dos treinos, paces e consistência.</p>
                </div>
                <div style="max-width:500px; margin-top:20px;">
                    <select id="ia-athlete-select" class="search-input"><option value="">Selecione o Aluno...</option></select>
                    <button class="btn btn-primary" onclick="AdminPanel.runGlobalIA()" style="margin-top:10px; width:100%;">Gerar Relatório de Performance</button>
                </div>
    ...
    renderFeedbackTable: () => {
        AdminPanel.state.db.ref('publicWorkouts').orderByChild('realizadoAt').limitToLast(100).once('value', snap => {
            ...
            let html = `<table class="sisrun-table">
                <thead><tr><th>Status</th><th>Aluno</th><th>Treino</th><th>Detalhes (GPS)</th><th>Ver</th></tr></thead><tbody>`;
    ...
    runGlobalIA: async () => {
        ...
        out.innerHTML = "Carregando dados e analisando...";
        try {
            ...
            const data = snap.val();
            const prompt = `Analise os treinos deste aluno. JSON: ${JSON.stringify(data)}. Fale sobre Pace e Volume. Use Markdown.`;
    ...
    // --- WORKSPACE ---
    renderAthleteList: (filter = "") => { /* Mesmo código anterior */
    ...
    loadWorkspaceWorkouts: (uid) => { /* Mesmo código timeline */
    ...
    renderPendingList: () => { /* Igual */ }
};

// AtletaPanel e FeedPanel permanecem no arquivo mas iguais ao anterior (omiti para poupar espaço, mas você deve manter o bloco final igual ao arquivo anterior ou usar o do App.js se preferir unificar).
